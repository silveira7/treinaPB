...
class Word:
    ...
    def __init__(self, word, pos, gramatica=None):
        ...
        if Word.homographs == None:
            Word.homographs = ExceptionHashMap(
                Path(__file__).parent / "Dictionaries" / "Homographs.json"
            )
            Word.homographs.create_hash()
        if Word.verbs == None:
            Word.verbs = ExceptionHashMap(
                Path(__file__).parent / "Dictionaries" / "Verbs.json"
            )
            Word.verbs.create_hash()
        if Word.exceptions == None:
            Word.exceptions = ExceptionHashMap(
                Path(__file__).parent / "Dictionaries" / "Exceptions.json"
            )
    ...
    def convert_word(self):
        ...
        if Word.exceptions.part_or_total(self.word) == 1:
            ...
            if Word.verbs.part_or_total(self.base) == 1:
                self.simplified = Word.phoneme_rules.simplify_word(
                    self.word, U_EXC=True
                )
            else:
                self.simplified = Word.phoneme_rules.simplify_word(self.word)
            self.find_tonic()
    ...
    def find_tonic(self):
        ...
        if self.tonic == -1:
            ...
            if vog1 in self.oxitonas:
                if (
                    vog2[-1] in ["i", "u"]
                    and self.word[pos2 + len(vog2)] not in self.vogais
                    and self.word[pos2 + len(vog2) + 1] not in self.vogais
                ):
